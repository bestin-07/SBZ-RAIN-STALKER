...
| **Drizzle surfacing** | light echo (0.1–0.5) beats a dry gauge → GO ANYWAY | capped at light band — cannot produce WAIT/STUCK; sets `drizzleSurfaced` which blocks gapNow; an RV-only claim (no filtered-nowcast echo) also requires an independent non-zero radar trace, not just "not clear sky" (v2.2.1 — an exact-zero radar reading is treated as disagreement regardless of cloud cover) | SOUND (was a live bug pre-v2.2.1: overcast + flat-zero radar + RV-only echo surfaced anyway) |
