...
- **Gewitter (thunderstorm, warntypid 5) is filtered out server-side** (`fetch_severe_warnings`, backend) — redundant with our own CAPE-based storm banner (`stormCape`/`regionalThunder` in App.jsx), which already covers that hazard from a different source. Never reaches `_ambient["warnings"]`, so it can't appear in the banner, the push, or the RED-headline override below.
