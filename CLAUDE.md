...
- **City centre only, once per 5-min cycle**, served as `_ambient["daily"]` — the same single-point precedent as `city_ground` and the warnings (a day-scale forecast does not vary meaningfully across 11 points 3 km apart). The browser makes **no call of its own**; the per-IP quota is untouched.
