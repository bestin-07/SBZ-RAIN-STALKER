...
```bash
cd frontend && npm test            # 313 tests in TWO files:
                                   #  gaps.test.js (301) — detectGaps, getStatus (all 4
                                   ...
                                   #  render.test.jsx (12) — the v2.30 display blocks mount
                                   #    and produce the right markup in BOTH languages; its
                                   #    t() THROWS on an unknown key, so a string added to
                                   #    only one language fails the suite
python backend/test_logic.py      # 26 tests: _filter_virga contract (heavy-pass!),
